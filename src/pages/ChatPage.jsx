...
export default function ChatPage() {
  const { user } = useAuth()
  const { selectedUser, selectedGroup, chatMode, selectUser, selectGroup } = useChat()
  // On mobile, show chat area only when a user or group is selected
  ...
  const showChat = chatMode === 'direct' ? selectedUser : selectedGroup
  ...
  return (
    <div className="h-screen flex flex-col items-center justify-center p-2 md:p-4 overflow-hidden fixed inset-0">
      ...
      <div className={`w-full max-w-5xl mb-2 flex items-center justify-center gap-2 ${showMobileChat && showChat ? 'hidden md:flex' : 'flex'}`}>
      ...
      <div className={`w-full max-w-5xl bg-dark-600/80 backdrop-blur-xl border border-primary-500/20 rounded-2xl shadow-2xl overflow-hidden flex ${showMobileChat && showChat ? 'h-screen md:h-[700px] rounded-none md:rounded-2xl' : 'h-[calc(100vh-4rem)] md:h-[700px]'}`}>
        
        {/* Sidebar - Full width on mobile, fixed width on desktop */}
        <div className={`
          ${showMobileChat && showChat ? 'hidden md:block' : 'block'}
